#!/usr/bin/env bash
# run_eval_3.sh — Install environment (if needed) then run Eval 3.
#
# Eval 3: type arbitrary sentences (a-z + space).
# 10 rollouts × max(0, 5 − Levenshtein(typed, target)) pts.
#
# Usage:
#   bash run_eval_3.sh [--sentences sentences.txt] [extra args]
#
# If --sentences is omitted, the built-in placeholder sentences are used.
...
if [[ ! -d "${VENV_DIR}" ]] || [[ ! -d "${KBD_VENV}" ]]; then
    echo "[run_eval_3] Environments not found — running setup_inference_pc.sh ..."
...
echo "[run_eval_3] Starting Roboflow inference server on port ${INFERENCE_PORT} ..."
...
_cleanup() {
    echo
    echo "[run_eval_3] Stopping inference server (PID ${INFERENCE_PID}) ..."
    kill "${INFERENCE_PID}" 2>/dev/null || true
    wait "${INFERENCE_PID}" 2>/dev/null || true
    echo "[run_eval_3] Done."
}
trap _cleanup EXIT INT TERM

# Wait until the server is ready (up to 30 s)
echo "[run_eval_3] Waiting for inference server to be ready ..."
for i in $(seq 1 30); do
    if curl -sf "http://localhost:${INFERENCE_PORT}/info" >/dev/null 2>&1; then
        echo "[run_eval_3] Inference server ready."
        break
    fi
    if (( i == 30 )); then
...
# ── Run Eval 3 ─────────────────────────────────────────────────────────────────
echo "[run_eval_3] Starting Eval 3: sentence typing ..."
"${VENV_DIR}/bin/python" "${REPO_DIR}/eval_keyboard_eval3.py" "$@"
