#!/usr/bin/env bash
# run_eval_2.sh — Install environment (if needed) then run Eval 2.
#
# Eval 2: press any a-z key given as input within 10 seconds.
# 16 rollouts × 3.125 pts = 50 pts max.
#
# Usage:
#   bash run_eval_2.sh [extra args forwarded to eval_keyboard_eval2.py]
...
if [[ ! -d "${VENV_DIR}" ]] || [[ ! -d "${KBD_VENV}" ]]; then
    echo "[run_eval_2] Environments not found — running setup_inference_pc.sh ..."
...
echo "[run_eval_2] Starting Roboflow inference server on port ${INFERENCE_PORT} ..."
...
_cleanup() {
    echo
    echo "[run_eval_2] Stopping inference server (PID ${INFERENCE_PID}) ..."
    kill "${INFERENCE_PID}" 2>/dev/null || true
    wait "${INFERENCE_PID}" 2>/dev/null || true
    echo "[run_eval_2] Done."
}
trap _cleanup EXIT INT TERM

# Wait until the server is ready (up to 30 s)
echo "[run_eval_2] Waiting for inference server to be ready ..."
for i in $(seq 1 30); do
    if curl -sf "http://localhost:${INFERENCE_PORT}/info" >/dev/null 2>&1; then
        echo "[run_eval_2] Inference server ready."
        break
    fi
    if (( i == 30 )); then
...
# ── Run Eval 2 ─────────────────────────────────────────────────────────────────
echo "[run_eval_2] Starting Eval 2: 16-rollout single-key press ..."
"${VENV_DIR}/bin/python" "${REPO_DIR}/eval_keyboard_eval2.py" "$@"
