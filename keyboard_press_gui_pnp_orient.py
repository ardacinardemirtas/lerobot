#!/usr/bin/env python3
"""
keyboard_press_gui_pnp_orient.py — Like keyboard_press_gui_pnp.py but the
arm returns to reset-home (wrist orientation held) after every key press.

Controls
--------
    Left-click on a key box    two-step PnP press → auto-return home
...
_WIN      = "SO-101  Keyboard Press  [PnP + Orient]"
...
class KeyboardPressGUI:
    ...
    def __init__(
        ...
        self._status       = "Ready — click a detected key to press it  [PnP + orient-hold]"
    ...
    def _press_worker(self, key_label: str) -> None:
        ...
        try:
            # lift=False: we handle the KB_HOME return ourselves
            press_key(
                key_label,
                self.robot,
                self.kin,
            ...
            if self._cancel_evt.is_set():
                return
            self._status = f"Pressed '{key_label}' — returning to KB_HOME …"
            if return_to_kb_home(self.robot):
                self._status = f"Pressed '{key_label}' — at KB_HOME"
            else:
                self._status = f"Pressed '{key_label}' — press F5 first to set KB_HOME"
        except Exception as exc:
    ...
    def run(self) -> None:
        ...
        print(f"{'─' * 55}")
        print("  SO-101 Keyboard Press GUI  [PnP + orient-hold]")
        ...
        print( "  After each press the arm returns to reset home (orient-hold).")
