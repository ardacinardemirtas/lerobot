...
wrist_roll is fixed at FIXED_WRIST_ROLL_DEG = -90°.
wrist_flex is IK-controlled but subject to an orientation task that keeps the
gripper tool axis pointing in the same world-frame direction it had at the
start of each move command.  The arm moves in 3-D position while the wrist
...
# All 5 arm joints used by ikpy FK (wrist_roll included for correct EE pose)
...
# Orientation-hold task
# Which axis of gripper_frame_link points along the tool fingers.
# Negate if the arm corrects in the wrong direction.
GRIPPER_TOOL_LOCAL = np.array([0.0, 0.0, 1.0])
ORIENT_GAIN        = 1.5   # rad/s per unit of tool-axis error
ORIENT_TASK_WEIGHT = 8.0   # QP weight — lower than QP_TASK_WEIGHT so position wins
...
class SO101Kinematics:
    """FK, position Jacobian, and orientation Jacobian for SO-101."""
    ...
    def __init__(self,
        ...
        # ikpy chain for FK — wrist_roll included so full EE pose is correct
    ...
    def forward_kinematics(self, joint_pos_deg: np.ndarray) -> np.ndarray:
        """Returns 4×4 EE transform using ikpy (all 5 joints contribute to pose)."""
    ...
    def full_jacobian(self,
                      ...
                      eps_rad: float = 1e-4) -> tuple[np.ndarray, np.ndarray]:
        """
        Returns (J_pos [3×n], J_orient [3×n]) in one finite-difference pass.
        J_orient columns are d(tool_axis_world)/dq_i — sensitivity of the gripper
        tool axis (GRIPPER_TOOL_LOCAL expressed in world frame) to each active joint.
        """
        names   = active_joints or self.active_joints
        n       = len(names)
        q       = np.asarray(joint_pos_deg, dtype=float)
        eps_deg = math.degrees(eps_rad)
        J_pos    = np.zeros((3, n), dtype=float)
        J_orient = np.zeros((3, n), dtype=float)
        for col, name in enumerate(names):
            ...
            T_minus = self.forward_kinematics(q_minus)
            J_pos[:, col]    = (T_plus[:3, 3]  - T_minus[:3, 3])  / (2.0 * eps_rad)
            tool_plus        = T_plus[:3, :3]  @ GRIPPER_TOOL_LOCAL
            tool_minus       = T_minus[:3, :3] @ GRIPPER_TOOL_LOCAL
            J_orient[:, col] = (tool_plus - tool_minus) / (2.0 * eps_rad)
        return J_pos, J_orient

    def position_jacobian(self,
                          joint_pos_deg: np.ndarray,
                          active_joints: list[str] | None = None,
                          eps_rad: float = 1e-4) -> np.ndarray:
        J_pos, _ = self.full_jacobian(joint_pos_deg, active_joints, eps_rad)
        return J_pos
...
def _qp_qdot(
    ...
    prev_qdot_rad_s: np.ndarray,
    dt: float,
    tool_axis_ref: np.ndarray,
) -> np.ndarray:
    J_pos, J_orient = kin.full_jacobian(q_deg, active_joints)
    ...
    # Orientation error: drive current tool axis toward the reference captured at move start
    R_curr    = kin.forward_kinematics(q_deg)[:3, :3]
    tool_curr = R_curr @ GRIPPER_TOOL_LOCAL
    v_orient  = ORIENT_GAIN * (tool_axis_ref - tool_curr)
    ...
    n = len(active_joints)
    H = (
        QP_TASK_WEIGHT    * (J_pos.T @ J_pos)
        + ORIENT_TASK_WEIGHT * (J_orient.T @ J_orient)
        + damping_weight  * np.eye(n)
        + QP_ACCEL_WEIGHT   * np.eye(n)
        + QP_POSTURE_WEIGHT * np.eye(n)
    )
    g = (
        -QP_TASK_WEIGHT    * (J_pos.T @ v_task)
        - ORIENT_TASK_WEIGHT * (J_orient.T @ v_orient)
...
def _controller_step(
    ...
    qdot  = _qp_qdot(kin, q_deg, v_task, kin.active_joints, q_ref_deg,
                     prev_qdot_rad_s, dt, tool_axis_ref)
...
def smooth_move(robot, kin: SO101Kinematics, target_pos: np.ndarray) -> dict:
    """
    Minimum-jerk Cartesian path + QP differential IK with orientation hold.

    At the start of each call the current gripper tool axis in world frame is
    recorded as the orientation reference.  The QP then applies a secondary
    task that drives the tool axis back toward that reference on every tick,
    keeping wrist posture constant despite the arm moving through space.

    wrist_roll is fixed at FIXED_WRIST_ROLL_DEG throughout; wrist_flex is
    free but regulated by the orientation task.
    """
    dt    = 1.0 / FPS
    obs   = robot.get_observation()
    q_obs = kin.clip_joints(_joints_from_obs(obs))
    # p_start and tool_axis_ref both derived from actual q so they are
    # consistent — no phantom initial error.
    T_obs          = kin.forward_kinematics(q_obs)
    p_start        = T_obs[:3, 3].copy()
    tool_axis_ref  = (T_obs[:3, :3] @ GRIPPER_TOOL_LOCAL).copy()
    ...
    print(f"  tool axis ref: [{tool_axis_ref[0]:+.3f}, {tool_axis_ref[1]:+.3f}, {tool_axis_ref[2]:+.3f}]")
    print(f"  Moving {distance * 100.0:.1f} cm  reference time {duration:.2f}s")
    print(f"  {'t(s)':>6}  {'err mm':>8}  {'orient°':>8}  {'sigma':>7}")
    ...
    while True:
        ...
        # orientation tracking error in degrees (angle between current and ref tool axis)
        R_actual   = kin.forward_kinematics(q_actual)[:3, :3]
        tool_curr  = R_actual @ GRIPPER_TOOL_LOCAL
        cos_angle  = float(np.clip(np.dot(tool_curr, tool_axis_ref), -1.0, 1.0))
        orient_err_deg = math.degrees(math.acos(cos_angle))

        if elapsed >= duration and final_error_m < SETTLE_THRESHOLD_M and joint_speed_deg < STOP_JOINT_SPEED_DEG_S:
            stable += 1
            if stable >= STABLE_TICKS:
                print(f"  done    t={elapsed:.3f}s  final={final_error_m*1000:.2f}mm  orient={orient_err_deg:.1f}°")
                return {"success": True, "final_error_mm": final_error_m * 1000.0,
                        "orient_error_deg": orient_err_deg, "elapsed_s": elapsed}
        else:
            stable = 0

        if elapsed - last_print >= 0.25:
            print(f"  {elapsed:6.3f}  {final_error_m*1000:8.2f}  {orient_err_deg:8.1f}  {info['sigma_min']:7.4f}")
            last_print = elapsed

        if elapsed > duration + MAX_SETTLE_S:
            print(f"  timeout t={elapsed:.3f}s  final={final_error_m*1000:.2f}mm  orient={orient_err_deg:.1f}°")
...
def run_repl(robot, kin: SO101Kinematics) -> None:
    print("SO-101 EE Controller (hold wrist orientation)  —  type 'help' for commands\n")

    while True:
        obs = robot.get_observation()
        q   = _joints_from_obs(obs)
        T   = kin.forward_kinematics(q)
        tool = T[:3, :3] @ GRIPPER_TOOL_LOCAL
        _print_ee(T)
        print(f"     tool axis: [{tool[0]:+.3f}, {tool[1]:+.3f}, {tool[2]:+.3f}]")
