...
# Gripper-down orientation constraint
# GRIPPER_TOOL_LOCAL: which axis of gripper_frame_link points along the tool fingers.
# If the arm swings the wrong way on orientation correction, negate this vector.
GRIPPER_TOOL_LOCAL  = np.array([0.0, 0.0, 1.0])
GRIPPER_DOWN_WORLD  = np.array([0.0, 0.0, -1.0])  # desired tool direction in world frame
ORIENT_GAIN         = 1.5    # orientation feedback gain (dimensionless/s → rad/s)
ORIENT_TASK_WEIGHT  = 6.0    # QP weight for orientation (lower than QP_TASK_WEIGHT so position wins)
...
# ── URDF kinematics (matches move_to_position_new.py exactly) ─────────────────
...
class SO101Kinematics:
    ...
    def __init__(self,
                 ...
                 fixed_joints: dict[str, float] | None = None) -> None:
        self.base_link     = base_link
        self.target_link   = target_link
        self.joints        = self._load_joints(Path(urdf_path))
        self.chain         = self._chain_to_target()
        self.fixed_joints  = fixed_joints or {}
        self.active_joints = [n for n in ARM_JOINTS if n in self.joints and n not in self.fixed_joints]
    ...
    def forward_kinematics(self, joint_pos_deg: np.ndarray) -> np.ndarray:
        ...
        q_by_name = {name: math.radians(float(q[idx])) for name, idx in JOINT_INDEX.items() if name in ARM_JOINTS}
    ...
    def bounds_deg(self, joint_names: list[str] | None = None) -> tuple[np.ndarray, np.ndarray]:
        """Accepts optional joint_names to match move_to_position_new.py's signature."""
    ...
    def position_jacobian(self,
        ...
        """3×n position Jacobian (kept for sigma_min display)."""
    ...
    def full_jacobian(self,
        ...
        """
        Returns (J_pos [3×n], J_orient [2×n]) in a single finite-difference pass.

        J_orient columns are d(tool_axis_world[:2])/dq_i — the XY sensitivity of the
        gripper tool axis direction to each joint.  Driving J_orient @ qdot toward
        -tool_axis_world[:2] pulls the gripper toward vertical.
        """
        ...
        J_orient = np.zeros((2, n), dtype=float)
        for col, name in enumerate(names):
            ...
            J_orient[:, col] = (tool_plus[:2] - tool_minus[:2]) / (2.0 * eps_rad)
...
def _qp_qdot(
    ...
    # Orientation error: XY components of tool axis in world frame → want them zero
    R_curr  = kin.forward_kinematics(q_deg)[:3, :3]
    tool_xy = (R_curr @ GRIPPER_TOOL_LOCAL)[:2]          # should approach [0, 0]
    v_orient = ORIENT_GAIN * (GRIPPER_DOWN_WORLD[:2] - tool_xy)  # correction velocity
...
def smooth_move(robot, kin: SO101Kinematics, target_pos: np.ndarray) -> dict:
    ...
    Key difference from the simulation version: qdot is integrated into a
    separate accumulator (q_cmd) rather than on top of q_actual.  On hardware
    the servo deadband means q_actual may not change for many ticks, so
    integrating on top of q_actual would produce the same tiny command every
    ...
    for name, value in kin.fixed_joints.items():   # enforce fixed joints before FK
        q_start[JOINT_INDEX[name]] = value
    ...
    q_ref   = q_start.copy()
    q_cmd   = q_start.copy()   # accumulated command — grows independently of q_actual
    ...
    while True:
        ...
        for name, value in kin.fixed_joints.items():   # re-enforce after integration
            q_cmd[JOINT_INDEX[name]] = value
...
def main() -> None:
    ...
    print(f"  All 5 joints active — gripper-down maintained via QP orientation constraint")
